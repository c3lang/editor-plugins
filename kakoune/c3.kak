...
# provide-module c3 %{
# TODO: macros are now highlighted as attribues, including @body()
...
evaluate-commands %sh{
    # TODO: maybe seperate into local only keywords (if) and global keywords (const)
    keywords='
	...
	addhl shared/c3/${region}/code/num        regex '(?<=[^A-z0-9])[+-]?(?:0(?:[xX][0-9a-fA-F](?:_|[0-9a-fA-F])*|[oO][0-7](?:_|[0-7])*|[bB][10](?:_|[10])*)|[0-9](?:_?[0-9])*(?:_?[eE][+-]?[0-9]+)?)(?:i8|i16|i32|i64|i128|u8|u16|u32|u|u64|u128|f|f32|f64)?\\b' 0:value
	addhl shared/c3/${region}/code/const-and-struct   regex '\\b[A-Z].*?\\b' 0:value
	...
	addhl shared/c3/global/code/function-statement   regex '(?:(?:fn|macro) +(?:[A-z0-9_\\[\\]]*\\**!? +)?(?:[^ \\t]+\\.)?)(@?[a-z]\\w+(?=\\())' 1:function
